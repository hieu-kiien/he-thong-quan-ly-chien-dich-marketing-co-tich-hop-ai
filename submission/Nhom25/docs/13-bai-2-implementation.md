...
| 4 | Tìm kiếm, lọc, sắp xếp | IMPLEMENTED_BASELINE | `campaign_list`: từ khóa, status, channel, khoảng ngày, sort; test lọc đúng và ngày sai không crash |
| 5 | Thống kê/báo cáo cơ bản | IMPLEMENTED_BASELINE | Dashboard tổng số campaign, active, pending review, clicks, cost; aggregate từ `Metric` và test KPI |
| 6 | Giao diện rõ ràng, dễ sử dụng | IMPLEMENTED_BASELINE | Django templates, CSS responsive, trạng thái rỗng, messages thành công/lỗi, confirm trước xóa, form validation |
...
| 8 | Xử lý lỗi cơ bản | IMPLEMENTED_BASELINE | Validation ngày/metric, 400 AI thiếu brief, 405 route chỉ POST, ProtectedError khi xóa channel đang được dùng |
| 9 | Minh chứng dùng AI khi lập trình | IMPLEMENTED_BASELINE | `evidence/ai/B2-CODE-001-v1-review.md` ghi input, phạm vi hỗ trợ, file đã sửa và cách kiểm chứng; evidence AI nghiệp vụ giữ riêng |
| 10 | Mã nguồn và tài liệu chạy thử | IMPLEMENTED_BASELINE | `README.md`, `marketing_management/README.md`, `.env.example`, `requirements.txt`, lệnh migrate/seed/test; commit cuối cần được chốt trước khi nộp |
...
- Provider AI thật chưa được gọi trong test tích hợp vì cần API key và endpoint
  do người dùng cung cấp; baseline dùng fallback offline.
