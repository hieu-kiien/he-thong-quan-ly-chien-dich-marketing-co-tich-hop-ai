...
\newcommand{\baseline}{\parbox[t]{\linewidth}{\scriptsize\raggedright IMPLEMENTED\\BASELINE}}
\newcommand{\baselineplus}[1]{\parbox[t]{\linewidth}{\scriptsize\raggedright IMPLEMENTED\\BASELINE\\#1}}
...
Hai nguồn P0 / CRITICAL là hai ảnh đề bài
\code{BÀI KIỂM TRA.png} và \code{DỰ ÁN.png} trong thư mục
\sourcefile{source-materials/}. Các nguồn triển khai được đối chiếu trong bảng
sau.
...
\midrule
\endhead
FR--001 & Đăng nhập/phân quyền & \shortstack[l]{username/\\password/\\role} & phiên đăng nhập, quyền & \baselineplus{một phần}\\
...
Test & \sourcefile{campaigns/tests.py} & 25 test model/permission/view/AI/CRUD/filter/dashboard/report/pagination/seed & \baseline\\
\bottomrule
\end{tabularx}
...
KT2 & sinh model/CRUD/test/debug & 25 test + \code{manage.py check}\\
...
Các test bao phủ aggregate metric, approval gate, fallback AI, provider contract,
permission guard, CRUD bốn nhóm dữ liệu, lọc keyword/channel/date/sort, phân trang,
filter sai không crash, dashboard KPI/report theo ngày và kênh, xử lý
\code{ProtectedError}, staff bị chặn route manager-only và \code{seed\_demo}
idempotent/console-safe.
...
\begin{longtable}{@{}P{9mm}P{44mm}P{35mm}P{62mm}@{}}
\toprule
...
4 & Tìm kiếm, lọc, sắp xếp & \baseline & keyword, status, channel, date range, sort và pagination; ngày sai không crash\\
5 & Thống kê/báo cáo & \baseline & dashboard lọc ngày, KPI CTR/conversion/cost và report/biểu đồ theo kênh\\
6 & Giao diện dễ dùng & \baseline & templates, CSS responsive, empty state, message, validation, confirm delete, ARIA\\
7 & CSDL và dữ liệu mẫu & \baseline & SQLite, migration, seed demo, constraint; test seed idempotent\\
8 & Xử lý lỗi cơ bản & \baseline & validation, lọc ngày sai, 400/405, protected delete và thông báo không crash\\
9 & Minh chứng AI lập trình & \baseline & \sourcefile{evidence/ai/B2-CODE-001-v1-review.md}, fallback/provider contract test\\
10 & Mã nguồn/tài liệu chạy thử & \baseline & README, \code{.env.example}, requirements, lệnh chạy/test, LaTeX/PDF/ZIP và Git history\\
\bottomrule
\end{longtable}
...
\sourcefile{evidence/ai/} cùng JSON đã khử dữ liệu. Kiểm tra template/test client
cho dashboard, ARIA và pagination được ghi tại
\sourcefile{evidence/runtime/B2-UI-001-v1-review.md}; không tuyên bố đã kiểm thử
mọi trình duyệt khi chưa có môi trường DevTools.
...
\midrule
RISK--001 & Provider ngoài chưa gọi thật & Cao & fallback + mock/contract; cần API key/endpoint để tích hợp\\
...
\begin{enumerate}
  ...
  \item Nếu được cấp endpoint, thêm integration test provider thật, injection, PII,
    timeout và schema lỗi.
  \item Đã chốt baseline; trước khi nộp kiểm tra \code{git status}, chạy lại checklist và đóng gói PDF/LaTeX/ZIP.
\end{enumerate}
